...
const SKIP_DIRS = new Set(["assets", "css", "libs", "tag", "search", "CLAUDE"]);
// Files to skip when indexing
...
function findHtml(folder) {
    ...
    for (var i = 0; i < files.length; i++) {
        ...
        else if (isHtml(filename) && !SKIP_FILES.has(files[i])){
            htmls.push(files[i]);
        };
    };
    return htmls;
...
function readHtml(root, file, fileId) {
    ...
    var body = $(".franklin-content").text() || $("body").text();
